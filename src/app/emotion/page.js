    ...
    <div className="max-w-[1800px] mx-auto p-4 md:p-12 space-y-8 md:space-y-12 relative z-10 min-h-[calc(100vh-80px)] overflow-hidden">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs md:text-base text-[#bc13fe] font-black uppercase tracking-widest mt-1">Real-time Emotion Telemetry</p>
        </div>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div className="relative z-10 rounded-2xl overflow-hidden border-2 border-dashed border-[#00f3ff]/30 group-hover:border-[#00f3ff]/60 transition-colors">
            {/* The actual webcam component (logic preserved) */}
          ...
          <div className="mt-4 flex justify-between items-center px-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#00f3ff]">Optical Sensor Active: Recording</span>
            </div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
              FRAME RATE: 30FPS • ENCRYPTED
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="glass-panel p-6 rounded-3xl border border-[#bc13fe]/20 shadow-[0_0_30px_rgba(188,19,254,0.05)] flex-1 relative overflow-hidden">
            ...
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 flex items-center gap-2 mb-6">
              <Activity size={16} className="text-[#bc13fe]" /> Live Telemetry
            </h2>
            
            {currentEmotion ? (
              <div className="space-y-6 relative z-10">
                <div className="text-center p-4 md:p-6 bg-white/5 rounded-2xl border border-white/10">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Detected State</p>
                  <p className="text-3xl md:text-4xl font-black capitalize text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
                    {currentEmotion.type === 'happy' ? '😊 Happy' :
                     currentEmotion.type === 'sad' ? '😢 Sad' :
                     currentEmotion.type === 'angry' ? '😠 Angry' :
                     currentEmotion.type === 'fear' ? '😨 Fear' :
                     currentEmotion.type === 'surprise' ? '😲 Surprised' :
                     currentEmotion.type === 'neutral' ? '😐 Neutral' :
                     currentEmotion.type}
                  </p>
                </div>
              ...
              <div className="h-40 flex flex-col items-center justify-center text-center opacity-50 relative z-10">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">Awaiting facial lock...</p>
              </div>
            )}
          </div>
          ...
          >
            ...
            {isSaving ? 'Logging Data...' : 'Commit to Ledger'}
          </motion.button>
          ...
          <AnimatePresence>
            {saveStatus && (
              ...
              >
                {saveStatus.includes('success') ? <CheckCircle2 size={16} /> : saveStatus.includes('Saving') ? <Activity size={16} className="animate-spin" /> : <AlertTriangle size={16} />}
                {saveStatus}
              </motion.div>
            )}
          </AnimatePresence>
